...
import { Disposable, PrebuildUpdate, WorkspaceInstance } from "@gitpod/gitpod-protocol";
...
export class PrebuildUpdateListener extends AbstractTopicListener<PrebuildUpdate> {

    constructor(protected readonly messageBusHelper: MessageBusHelper, listener: TopicListener<PrebuildUpdate>, protected readonly projectId?: string) {
        super(messageBusHelper.workspaceExchange, listener);
    }
...
export class MessageBusIntegration extends AbstractMessageBusIntegration {
    ...
    listenForPrebuildUpdates(
        callback: (ctx: TraceContext, evt: PrebuildUpdate) => void,
    ...
    async notifyOnPrebuildUpdate(prebuildInfo: PrebuildUpdate) {
        if (!this.channel) {
            throw new Error("Not connected to message bus");
        }
        const topic = `prebuild.update.project-${prebuildInfo.prebuildInfo.projectId}`;
        await this.messageBusHelper.assertWorkspaceExchange(this.channel);
