...
export class Server<C extends GitpodClient, S extends GitpodServer> {
    ...
    public async init(app: express.Application) {
        ...
        log.info('oldConfig', { oldConfig: JSON.stringify(configFromEnv, undefined, 2) });
        (() => {
            if (JSON.stringify(this.config) !== JSON.stringify(configFromEnv)) {
                log.error("config mismatch: new config does not match new one!");
                process.exit(1)
            }
        })();
        ...
        this.eventEmitter.on(Server.EVENT_ON_START, (httpServer) => {
            ...
            const csrfGuard: ws.VerifyClientCallbackAsync = (info: { origin: string; secure: boolean; req: http.IncomingMessage }, callback: (res: boolean, code?: number, message?: string) => void) => {
                let allowedRequest = isAllowedWebsocketDomain(info.origin, this.config.hostUrl.url.hostname);
